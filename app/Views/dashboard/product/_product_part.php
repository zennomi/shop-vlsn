        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (obj.result == 1 && obj.htmlContent != '') {
                    var selectTag = '<select class="form-control subcategory-select custom-select" data-level="' + newLevel + '" name="category_id[]" onchange="getSubCategoriesDashboard(this.value,' + newLevel + ',' + langId + ');">' +
                        '<option value=""><?= trans('select_category'); ?></option>' + obj.htmlContent + '</select>';
